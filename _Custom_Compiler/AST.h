...
struct LiteralExpr : Expr
{
    ...
    {
        cout << string(indent, ' ') << "LiteralExpr(" << type << ": " << value << ")\n";
    }
};
...
struct IdentifierExpr : Expr
{
    ...
    void print(int indent = 0) const override
    {
        cout << string(indent, ' ') << "IdentifierExpr(" << name << ")\n";
    }
};
...
struct UnaryExpr : Expr
{
    string op;
    Expr* right;
    UnaryExpr(const string& o, Expr* r) : op(o), right(r) {}
    ~UnaryExpr()
    {
        delete right;
    }
...
struct ForStmt : Stmt // for loop
{
    ...
    Stmt* condition;   // e.g i < 10;
    Expr* update;   // e.g i++ or i+=1;
    Stmt* body;       // loop body
    ForStmt(Stmt* init, Stmt* cond, Expr* upd, Stmt* b)
...
struct funcDecl : Stmt
{
    ...
    funcDecl(const string& retType, const string& n, const vector<string>& p, Block* b = nullptr)
        : returnType(retType), name(n), params(p), body(b) {
    }
    ...
    void print(int indent = 0) const override
    {
        cout << string(indent, ' ') << "funcDecl(" << returnType << " " << name << ")\n";
